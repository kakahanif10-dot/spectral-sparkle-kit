// PURPLE — Autonomous AI Consultant runtime helpers.
...
function buildProjectFiles(spec: DesignSpec): { name: string; content: string }[] {
  ...
  const catalog =
    `// Industry catalog generated by the Purple Context-Aware Engine.\n` +
  ...
  const appJson = JSON.stringify(
    {
      expo: {
        ...
        android: { package: `build.purple.${slug.replace(/-/g, '')}` },
      },
    },
    null,
    2,
  ...
  const readme = [
    ...
    `${spec.description}`,
    ``,
    `Generated by the **Purple Context-Aware Engine** as a real Expo /`,
    ...
    `> so Purple exports the honest, buildable source instead of a broken link.`,
    ``,
  ].join('\n')
